...
Napi::Value PyObj::ToJS(Napi::Env env, PyObject *py) {
  ...
  if (PyLong_Check(py)) { return scope.Escape(Number::New(env, PyLong_AsLong(py))); }

  if (PyFloat_Check(py)) { return scope.Escape(Number::New(env, PyFloat_AsDouble(py))); }
  ...
  if (PyUnicode_Check(py)) {
    ...
    return scope.Escape(String::New(env, reinterpret_cast<char16_t *>(raw + 2), PyUnicode_GET_LENGTH(py)));
  }
